...
from loreleai.language.lp import c_var, c_pred, Clause, Atom, Body, Not, List, c_const,Structure
from loreleai.learning.hypothesis_space import TopDownHypothesisSpace
from loreleai.learning.language_filtering import has_duplicated_literal, has_duplicated_variable,connected_body
...
import heapq as has
from MyModel import encoding2,decoding
from MyModel import Network
import numpy as np
import random

# Search for problem b45
class MyOwnLearner:
...
if __name__ == '__main__':
    ...
    primitives2 = [b45(c_var('X')), b45(c_var('Y')), not_space(c_var('X')), not_space(c_var('Y')),
                   ...
                   mk_lowercase(c_var('Y'), c_var('Z')), is_empty(c_var('X')), is_empty(c_var('Y')),
                   is_space(c_var('X'))
    ...
    learner = MyOwnLearner(prolog)

    test_set = [Atom(b45, [Structure(s, [List(
    ...
    print('coverage clause 1 : ', learner.evaluateClause(test_set, clause1))
    body1 = Atom(copyskip1, [A,B])
    body2 = Atom(skip1, [B,D])
    body3 = Atom(skip1, [C, D])
    body4 = Atom(mk_lowercase, [C,F])
    ...
    print('coverage clause 2 : ', learner.evaluateClause(test_set, clause2))
