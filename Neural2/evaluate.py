...
from loreleai.language.lp import c_var, c_pred, Clause, Procedure, Atom, Body, Not, List, c_const, c_functor, c_literal, Structure,Predicate,Variable, Type
...
import random
from Neural2 import encoding2, encoding
...
class evaluate:
    ...
    def generateExamples(self, mostgenericclause,positive_examples,k,encoder:encoding2):
        ...
        network2 = Network.Network.LoadNetwork('/home/marlon/loreleai/Neural2/network(b291-2a)28.txt')
        network3 = Network.Network.LoadNetwork('/home/marlon/loreleai/Neural2/network(b291-1b)28.txt')
        network1 = Network.Network.LoadNetwork('/home/marlon/loreleai/Neural2/network(b291-1a)28.txt')
        network4 = Network.Network.LoadNetwork('/home/marlon/loreleai/Neural2/network(b291-2b)28.txt')
        ...
        for state in [1]:
            ...
            while (len(trainingprograms)!=0):
                ...
                if len(clause.get_literals())<4:
                    ...
                    if len(clause.get_literals()) < 5:
                        ...
                        for training in bestk:
                            trainingprograms.append(training)
                    #else:
                    #    if len(clause.get_literals()) < 6:
                    #        bestk = self.pickKBest(sorted(expansions[1], key=lambda l: l[1], reverse=True), k - 2)
                    #        for training in bestk:
                    #            trainingprograms.append(training)
                ...
                if (self._number % 49 == 0 ) &(len(input)>0):
                    #string1 = str(state) + 'input' + str(self._number) + '.txt'
                    #string2 = str(state) + 'output' + str(self._number) + '.txt'
                    #self.writeEncodingsToFile(input, string1)
                    #self.writeEncodingsToFile(output, string2)
                    print('train 1')
                    tic = time.perf_counter()
                    network1.Train(input, output, 1)
                    network1.SaveNetwork('network(b291-1a)29.txt')
                    toc = time.perf_counter()
                    print(f"Downloaded the tutorial 2 in {toc - tic:0.10f} seconds")
                    print('train 1')
                    tic = time.perf_counter()
                    network3.Train(input, output, 1)
                    network3.SaveNetwork('network(b291-1b)29.txt')
                    toc = time.perf_counter()
                    print(f"Downloaded the tutorial 2 in {toc - tic:0.10f} seconds")
                    input = []
                    output = []
                ...
                if (self._number % 500 == 0) &(len(input)>0):
                    ...
                    print('train 2')
                    tic = time.perf_counter()
                    network2.Train(self._input2, self._output2, 1)
                    network2.SaveNetwork('network(b291-2a)29.txt')
                    toc = time.perf_counter()
                    print(f"Downloaded the tutorial in {toc - tic:0.10f} seconds")
                    print('train 2')
                    tic = time.perf_counter()
                    network4.Train(self._input2, self._output2, 1)
                    network4.SaveNetwork('network(b291-2b)29.txt')
                    toc = time.perf_counter()
                    print(f"Downloaded the tutorial in {toc - tic:0.10f} seconds")
                    self._input2 = []
                    self._output2 = []
    ...
    def generateExamples2(self, mostgenericclause, positive_examples, k, encoder: encoding2):
        ...
        self._encoder = encoder
        self._numberOfEvaluations = 0
        #network2 = Network.Network.LoadNetwork('/home/marlon/loreleai/Neural2/network3.txt')
        network = Network.Network.LoadNetwork('/home/marlon/loreleai/Neural2/network7.txt')
        print(states)
        for state in states:
            # state = [1,0,0,0,0]
            numberIO = 0
            ...
            while (len(trainingprograms) != 0):
                ...
                expansions = self.expand(clause, positive_examples_state)
                if len(clause.get_literals()) < 5:
                    bestk = self.pickK(sorted(expansions[1], key=lambda l: l[1]), k)
                    print(bestk)
                    for training in bestk:
                        trainingprograms.append(training)
                if len(clause.get_body().get_literals()) != 0:
                ...
                if (len(trainingprograms) == 0):
                    ...
                    network.SaveNetwork('network18.txt')
                    toc = time.perf_counter()
                    print(f"Downloaded the tutorial 2 in {toc - tic:0.10f} seconds")
                    input = []
                    output = []
                ...
                print('number of evaluations: ', len(self._input2))
            print(len(trainingprograms))
        self.writeEncodingsToFile(input, 'input.txt')
        self.writeEncodingsToFile(output, 'output.txt')
        tic = time.perf_counter()
        network.Train(input, output, 1)
        toc = time.perf_counter()
        print(f"Downloaded the tutorial in {toc - tic:0.10f} seconds")
        network.SaveNetwork('network.txt')
    ...
    def pickK(self,expansions,k=3):
        ...
        for clause in expansions :
            ...
            if clause[1] == best:
                ...
                if different == 1:
                    bestk.append(random.sample(last,1))
                    different = 0
                else:
                    if len(bestk) + len(last) <= k :
                        bestk = bestk + last
                        if (len(bestk)==k) & different == 0 :
                            return bestk
                        last = [clause]
                        best = clause[1]
                    else:
    ...
    def generateStates(self):
        ...
        for i in range(0,1):
            #alreadyHave = {[]}
            state = []
            #while alreadyHave.__contains__(state):
            newstate = [0, 0, 0, 0, 0]
...
if __name__ == '__main__':
    ...
    encoding1 = encoding.encoding(primitives)
    encodingExamples = {
    ...
    #exps = ev.expand(Clause(head, []), pos)
    #prim = lambda x: plain_extension(x, mk_uppercase, connected_clauses=True)

    #print(len(exps))
    #print(exps[1])
    #print(ev.generateStates())
    ...
    ev.generateExamples((Clause(head,[]),0), positive2, 4,encoding2)
     #input = ev.readEncodingsOfFile('input10.txt')
    #output = ev.readEncodingsOfFile('output10.txt')
    #network = Network.Network(3, [2212, 10, 17])
    #network.Train(input, output, 1)
    #network2 = Network.Network.LoadNetwork('network3.txt')
